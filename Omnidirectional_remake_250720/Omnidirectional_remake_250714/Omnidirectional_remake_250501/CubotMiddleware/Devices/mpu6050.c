...
float MPU6050_GYRO_SEN  = MPU6050_GYRO_500_SEN;//MPU6050_GYRO_1000_SEN;
...
{
    return Sensors_I2C_WriteRegister(&hi2c2,MPU6050_ADDRESS, reg_add, 1, &reg_dat);
}

// 从MPU6050寄存器读取数据
static uint8_t MPU6050_ReadData(uint8_t reg_add, uint8_t *Read, uint8_t num)
{
    return Sensors_I2C_ReadRegister(&hi2c2,MPU6050_ADDRESS, reg_add, num, Read);
}
