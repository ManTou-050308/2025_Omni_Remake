...
/*-------------- 遇到的代码阅读问题 ----------------*/
/*
    1.不同于正点原子的CAN配置例程，多了许多不一样的，如链表、CAN初始化、基本配置位置不一样、新建函数的笼扩性。
		2.HAL_FDCAN_RxFifo0Callback（）函数在fdcan中有HAL库的使用，但没看明白其作用是什么,像是一个判断过滤器接收到函数与否
		3.初始化链表还是不会写，看不懂其指针
		4.(*CAN_RxCpltCallback)(CAN_RxBuffer* rxBuffer) 用户回调函数难以理解
...
/*------------------代码阅读问题个人解析------------------*/
/*Date 2024-7-14  Author  ManTouJiang
	1.
		焦库运用了开发式的规范写法，是对HAL库的再封装从底层开始驱动层、设备层、应用层
	驱动层以CAN举例，在CubeMax中配置的io口作为CAN通讯使用，只是对基础的数据传输速度等传输途径
	在封装中会进行进一步的配置如过滤器配置的数据筛选途径，发送某一数据的的通道匹配。
		再谈及笼栝性，强烈建议去找其他兵种的代码看看，在你第一次看兵种代码就是这样规范的，
	会被限制在焦库中。在后面的设备层中会发现这种写法的底层性很强。
	2.
	3.
		创建链表，其个元素为指向该设备列表的指针，其可用于全遍历
	4.
		创建函数指针类型，其参量为指向接收缓存区的指针。在后续中可能会出现这样的用法：
			定义一个回调处理函数对接收数据进行处理，在将创建的函数指针指向他，在后面的再调用回调处理函数时，
		便可以直接使用函数指针进行替换使用。
